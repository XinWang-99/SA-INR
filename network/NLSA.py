...
from utils import BasicBlock, batched_index_select
...
class NonLocalSparseAttention(nn.Module):
    ...
    def __init__(self,
        ...
        self.conv_match = BasicBlock(channels,
                                     channels // reduction,
                                     k_size,
                                     bn=False,
                                     act=None)
        self.conv_assembly = BasicBlock(channels,
                                        channels,
                                        1,
                                        bn=False,
                                        act=None)
    ...
    def LSH(self, hash_buckets, x):
        ...
        rotations_shape = (1, x.shape[-1], self.n_hashes, hash_buckets // 2
                           )  #[1,C,n_hashes,hash_buckets//2]
        random_rotations = torch.randn(
            rotations_shape, dtype=x.dtype,
    ...
    def add_adjacent_buckets(self, x):
        ...
        x_extra_forward = torch.cat([x[:, :, 1:, ...], x[:, :, :1, ...]],
                                    dim=2)
        return torch.cat([x, x_extra_back, x_extra_forward], dim=3)

    def forward(self, input):

        N, _, H, W, D = input.shape
        x_embed = self.conv_match(input).view(N, -1,
                                              H * W * D).contiguous().permute(
                                                  0, 2, 1)
        y_embed = self.conv_assembly(input).view(
            N, -1, H * W * D).contiguous().permute(0, 2, 1)
        L, C = x_embed.shape[-2:]
        ...
        hash_codes = self.LSH(hash_buckets, x_embed)  #[N,n_hashes*H*W*D]
        hash_codes = hash_codes.detach()

        #group elements with same hash code by sorting
        _, indices = hash_codes.sort(dim=-1)  #[N,n_hashes*H*W*D]
        _, undo_sort = indices.sort(
            dim=-1)  #undo_sort to recover original order
        mod_indices = (indices % L)  #now range from (0->H*W*D)
        x_embed_sorted = batched_index_select(
            x_embed, mod_indices)  #[N,n_hashes*H*W*D,C]
        y_embed_sorted = batched_index_select(
            y_embed, mod_indices)  #[N,n_hashes*H*W*D,C]
        ...
        x_att_buckets = torch.reshape(
            x_embed_sorted, (N, self.n_hashes, -1, C))  #[N, n_hashes, H*W*D,C]
        y_att_buckets = torch.reshape(
            y_embed_sorted, (N, self.n_hashes, -1, C * self.reduction))
        if padding:
        ...
        x_att_buckets = torch.reshape(
            x_att_buckets, (N, self.n_hashes, -1, self.chunk_size,
        ...
        raw_score = torch.einsum(
            'bhkie,bhkje->bhkij', x_att_buckets,
        ...
        bucket_score = torch.reshape(bucket_score, (
            N,
            -1,
        ))  #[N,n_hashes*H*W]
        ...
        ret = ret.permute(0, 2, 1).view(
            N, -1, H, W, D).contiguous() * self.res_scale + input
        return ret
...
class NonLocalAttention(nn.Module):
    ...
    def __init__(self,
        ...
        self.res_scale = res_scale
        self.conv_match1 = BasicBlock(channel,
                                      channel // reduction,
                                      1,
                                      bn=False,
                                      act=nn.PReLU())
        self.conv_match2 = BasicBlock(channel,
                                      channel // reduction,
                                      1,
                                      bn=False,
                                      act=nn.PReLU())
        self.conv_assembly = BasicBlock(channel,
                                        channel,
                                        1,
                                        bn=False,
                                        act=nn.PReLU())
    ...
    def forward(self, input):
        ...
        return x_final.permute(0, 2, 1).view(N, -1, H,
                                             W) + self.res_scale * input
