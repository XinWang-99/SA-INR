...
from utils import batched_index_select,BasicBlock

class NonLocalSparseAttention(nn.Module):
    def __init__( self, n_hashes=4, channels=64, k_size=3, reduction=4, chunk_size=544, res_scale=1):
        super(NonLocalSparseAttention,self).__init__()
        self.chunk_size = chunk_size
        self.n_hashes = n_hashes
        self.reduction = reduction
        self.res_scale = res_scale
        self.conv_match = BasicBlock(channels, channels//reduction, k_size, bn=False, act=None)
        self.conv_assembly =BasicBlock(channels, channels, 1, bn=False, act=None)
    ...
    def forward(self, input):
        ...
        x_embed = self.conv_match(input).view(N,-1,H*W*D).contiguous().permute(0,2,1)
        y_embed = self.conv_assembly(input).view(N,-1,H*W*D).contiguous().permute(0,2,1)
        L,C = x_embed.shape[-2:]
        ...
        x_embed_sorted =batched_index_select(x_embed, mod_indices) #[N,n_hashes*H*W*D,C]
        y_embed_sorted =batched_index_select(y_embed, mod_indices) #[N,n_hashes*H*W*D,C]
        ...
        x_att_buckets = torch.reshape(x_embed_sorted, (N, self.n_hashes,-1, C)) #[N, n_hashes, H*W*D,C]
        ...
        ret = ret.permute(0,2,1).view(N,-1,H,W,D).contiguous()*self.res_scale+input
        return ret
