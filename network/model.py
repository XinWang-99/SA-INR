...
try:
    ...
    from network.edsr import conv_3d, make_edsr_baseline
...
class SA_INR(nn.Module):
    def __init__(self, conv=conv_3d, n_resblocks=8, n_feats=64, win_size=(7, 7, 2),layerType='FBLA', dilation=1\
        ...
        self.encoder = make_edsr_baseline(conv,
                                          n_resblocks,
                                          n_feats,
                                          add_atten=False)
        ...
        self.attentionLayer = AttentionLayer(n_feats, win_size, layerType,
                                             dilation)
        self.imnet = MLP(in_dim=n_feats,
                         out_dim=1,
    ...
    def inference(
        self, inp, feat, hr_coord, proj_coord
    ):  # inp (b,1,w/2,h/2,d/2)  # hr_coord (b,w*h*d,3) # proj_coord (b,w*h*d,3)
        ...
        q_feat = F.grid_sample(
            feat,
            hr_coord.flip(-1).view(bs, 1, 1, sample_q, 3),
            mode='bilinear',
            align_corners=True)[:, :, 0, 0, :].permute(
                0, 2, 1)  # q_feat (bs,c,sample_q)->(bs,sample_q,c)

        if not self.add_branch:
            mask = torch.cat([
                torch.zeros((bs, sample_q, 1)),
                torch.ones((bs, sample_q, 1))
            ],
                             dim=-1).to(inp.device)
        else:
            mask_p = self.mask_predictor(q_feat)  # mask (b,n,2)
            if self.is_train:
                mask = F.gumbel_softmax(mask_p, tau=1, hard=True,
                                        dim=2)  # return 2-dim one-hot tensor
            else:
        ...
        # branch 1
        idx_easy = torch.nonzero(mask[:, :,
                                      0].view(-1)).squeeze(1)  # idx_easy (m1)
        if torch.sum(mask[:, :, 0]) > 0:
            feat_easy = torch.index_select(q_feat.contiguous().view(
                -1, q_feat.shape[-1]), 0, idx_easy)  # feat_easy (m1,c)
            #feat_easy=self.FFNLayer1(feat_easy)
            pred_easy = self.imnet(feat_easy)  # pred_easy (m1,1)

        # branch 2
        idx_difficult = torch.nonzero(mask[:, :, 1].view(-1)).squeeze(
            1)  # idx_difficult (m2)
        if torch.sum(mask[:, :, 1]) > 0:
            ...
            for i in range(bs):
                if torch.sum(mask[i, :, 1]) == 0:
                    continue
                idx_each = torch.nonzero(mask[i, :, 1].view(-1)).squeeze(
                    1)  # idx_each (m2/)
                hr_coord_each = torch.index_select(
                    hr_coord[i], 0,
                    idx_each).unsqueeze(0)  # hr_coord_each(1,m2/,3)
                proj_coord_each = torch.index_select(
                    proj_coord[i], 0,
                    idx_each).unsqueeze(0)  # proj_coord_each (1,m2/,3)

                q_feat_each = torch.index_select(
                    q_feat[i], 0, idx_each).unsqueeze(0)  # feat_each (1,m2/,c)
                feat_each = self.attentionLayer(q_feat_each,
                                                feat[i].unsqueeze(0),
                                                proj_coord_each,
                                                hr_coord_each).squeeze(0)
                #feat_each=self.FFNLayer2(feat_each)
                pred_each = self.imnet(feat_each)  # (m2/,1)
                pred_difficult.append(pred_each)

            pred_difficult = torch.cat(pred_difficult,
                                       dim=0)  # pred_difficult (m2,1)
        # combine and scatter
        ...
        if torch.sum(mask[:, :, 0]) == 0:
            ...
            idx = torch.cat([idx_easy,
                             idx_difficult]).unsqueeze(1)  # idx (n,1)
        ...
        return {"pred": pred, "sparsity": sparsity, "mask": mask[:, :, 1]}

    def forward(
        self, inp, hr_coord, proj_coord
    ):  # inp (b,1,w/2,h/2,d/2)  # hr_coord (b,w*h*d,3) # proj_coord (b,w*h*d,3)
