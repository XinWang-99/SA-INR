...
class AttentionLayer(torch.nn.Module):
    ...
    def __init__(self, n_feats, win_size, layerType, dilation=1):
        ...
        if layerType == 'PBLA':
            self.pos_embedding = nn.Sequential(nn.Linear(3, 256),
                                               nn.Linear(256, 1))
        else:
        ...
        self.pad = torch.tensor([win_size[i] // 2
                                 for i in range(3)])  # (3,3,1)
        self.pad_list = torch.tensor([
            self.win_size[2] // 2, self.win_size[2] // 2,
            self.win_size[1] // 2, self.win_size[1] // 2,
            self.win_size[0] // 2, self.win_size[0] // 2
        ])  # (1,1,3,3,3,3)
    ...
    def projection(self, matrix, coord):
        ...
        # print(matrix.shape)
        return matrix[ind, :, coord[:, :, 0].view(-1), coord[:, :, 1].view(-1),
                      coord[:, :, 2].view(-1)].view(b, N, -1)

    def get_offset_coord(self, win_size):
        half_win_size = [math.ceil(win_size[i] * 0.5) - 1 for i in range(3)]
        offset = [
            torch.arange(-half_win_size[i], win_size[i] - half_win_size[i], 1)
            for i in range(3)
        ]
        # print(offset[1].shape)
        offset1 = offset[1].unsqueeze(0).repeat(win_size[2],
                                                1).permute(1,
                                                           0).reshape(-1, 1)
        offset2 = offset[2].unsqueeze(1).repeat(win_size[1], 1)
        offset12 = torch.cat([offset1, offset2], dim=1)
        offset0 = offset[0].unsqueeze(0).repeat(win_size[1] * win_size[2],
                                                1).permute(1,
                                                           0).reshape(-1, 1)
    ...
    def local_attention(
        self, q_feat, feat, proj_coord, hr_coord
    ):  # feat (b,c,w/2,h/2,d/2) q_feat(b,N,64) proj_coord(b,N,3) hr_coord (b,N,3)
        ...
        pad[:
            2] = pad[:2] * self.dilation  # (3,3,1)-> # (3*dilation,3*dilation,1)
        ...
        offset_coord = offset_coord.view(1, 1, *offset_coord.shape).expand(
            b, N, *offset_coord.shape)
        ...
        local_coord = local_coord.view(b, -1, 3).type(
            torch.long)  # (b,N*win_size[0]*win_size[1]*win_size[2],3)
        ...
        local_feat = self.projection(padded_feat, local_coord).view(
            b, -1, win_size[0] * win_size[1] * win_size[2],
            c)  # (b,N,win_size[0]*win_size[1]*win_size[2],64)

        if self.layerType == 'PBLA':
            lr_coord = make_coord(feat.shape[2:]).permute(3, 0, 1,
                                                          2).to(feat.device)
            lr_coord = lr_coord.unsqueeze(0).expand(
                b, *lr_coord.shape)  # (b,3,w/2,h/2,d/2)
            padded_coord = padding(lr_coord)
            local_pos = self.projection(padded_coord, local_coord).view(
                b, -1, win_size[0] * win_size[1] * win_size[2],
                3)  # (b,N,win_size[0]*win_size[1]*win_size[2],3)
            relative_pos = local_pos - hr_coord.unsqueeze(
                -2)  # (b,N,win_size[0]*win_size[1]*win_size[2],3)
            embedded_pos = self.pos_embedding(
                relative_pos)  # (b,N,win_size[0]*win_size[1]*win_size[2],1)
            ...
            atten = torch.einsum(
                'ijk,ijmk->ijm', q,
                k)  # (b,N,win_size[0]*win_size[1]*win_size[2])
